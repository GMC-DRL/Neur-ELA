from metabbo.learnable_optimizer import Learnable_Optimizer
import torch
import numpy as np
from ela_feature.ela_feature import get_ela_feature
...
class GLEET_Optimizer(Learnable_Optimizer):
    ...
    def init_population(self, problem):
        ...
        self.max_velocity = 0.2 * (problem.ub - problem.lb)
        ...
        # get state
        if self.fe is None:
            if self.__config.use_ela:
                state = self._get_ela_feature(problem)
            else:
            ...
            state = self.fe(self.particles['current_position'], self.particles['c_cost'])[0]   # ps, node_dim
            self.pbest_feature = state.clone()                                               # ps, node_dim
            
            self.gbest_feature = state[self.particles['gbest_index']]                       # node_dim

            gp_cat = self.torch_gp_cat()
            return torch.cat((state, gp_cat), dim=-1)           # ps, 3 * node_dim
        
        
    def _get_ela_feature(self, problem):
        ela = get_ela_feature(problem, self.particles['current_position'], self.particles['c_cost'])
        consumed_fes = ela[1]
        if self.__config.count_ela_fes:
            self.fes += consumed_fes
        state = ela[0][None, :].repeat(self.ps, axis=0)
        return state
    ...
    def torch_gp_cat(self):
        return torch.cat((self.pbest_feature,self.gbest_feature[None,:].repeat(self.ps, 1)),dim=-1)            # ps, 2 * node_dim
    ...
    def update(self, action, problem):
        ...
        if self.fe is None:
            # get next state
            if self.__config.use_ela:
                next_state = self._get_ela_feature(problem)
            else:
            ...
            next_state = self.fe(self.particles['current_position'], self.particles['c_cost'])[0]

            self.pbest_feature = torch.where(torch.tensor(self.per_no_improve[:,None]==0), next_state, self.pbest_feature)
            
            # update exploitation state
            if self.no_improve==0:
                self.gbest_feature=next_state[self.particles['gbest_index']]
            
            next_gpcat = self.torch_gp_cat()
            next_state = torch.cat((next_state, next_gpcat), dim=-1)

...
class LDE_Optimizer(Learnable_Optimizer):
    ...
    def init_population(self, problem):
        ...
        return self.__get_feature(problem)
    ...
    def __get_feature(self, problem):
        if self.fe is not None:
            input_net = self.fe(self.__pop.squeeze(0),self.__fit.squeeze(0)).mean(1)
        else:
            if self.__config.use_ela:
                ela = get_ela_feature(problem, self.__pop.squeeze(0),self.__fit.squeeze(0))
                consumed_fes = ela[1]
                if self.__config.count_ela_fes:
                    self.fes += consumed_fes
                input_net = ela[0][None, :]
            else:

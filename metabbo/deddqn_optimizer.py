...
from metabbo.deddqn_related.operators import rand_1_single, rand_2_single, rand_to_best_2_single, cur_to_rand_1_single, binomial, clipping
    ...
    def __get_state(self, problem):
        if self.fe is None:
            if self.__config.use_ela:
                if self.__pointer == 0:
                    ela = get_ela_feature(problem, self.__X, self.__cost)
                    consumed_fes = ela[1]
                ...
                return self.__origin_get_state(problem)
        else:
            if self.__pointer == 0:
                self.features = self.fe(self.__X, self.__cost)[0].numpy()
            return self.features[self.__pointer]

    def __origin_get_state(self, problem):

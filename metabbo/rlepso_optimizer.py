...
class RLEPSO_Optimizer(Learnable_Optimizer):
    ...
    def __reinit(self, filter, problem):
        ...
        new_particles = {'current_position': new_position,  # bs, ps, dim
                         ...
                         'gbest_val': float(np.where(filters_best_val,
                                               new_cbest_val,
    ...
    def __get_state(self, problem):
        ...
        if self.fe is None:
            ...
            # since RLEPSO query state for the whole population, we average individual feature
            indiv_features = self.fe(self.__particles['current_position'], self.__particles['c_cost'])[0]
            return indiv_features.mean(0)

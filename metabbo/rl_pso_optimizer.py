...
from metabbo.boundary_control import clipping
...
class RL_PSO_Optimizer(Learnable_Optimizer):
    ...
    def init_population(self, problem):
        ...
        return self.__get_state(self.__cur_index, problem)

    def __get_state(self, index, problem):
        if self.fe is not None:
            if index == 0:
                self.features = self.fe(self.__particles['current_position'], self.__particles['c_cost'])[0]
            return self.features[index]
        else:
            if self.__config.use_ela:
                if index == 0:
                    ela = get_ela_feature(problem, self.__particles['current_position'], self.__particles['c_cost'])
                    consumed_fes = ela[1]
    ...
    def update(self, action, problem):
        ...
        if is_done:
            if len(self.cost) >= self.__config.n_logpoint + 1:
                ...
                self.cost.append(self.__particles['gbest_val'])
        return self.__get_state(self.__cur_index, problem), reward, is_done

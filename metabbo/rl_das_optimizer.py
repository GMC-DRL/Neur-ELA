...
import numpy as np
from metabbo.rldas_related.optimizer import JDE21, MadDE, NL_SHADE_RSP

from metabbo.rldas_related.Population import *
import warnings
import copy
...
class RL_DAS_Optimizer(Learnable_Optimizer):
    ...
    def observe(self, problem):
        ...
        if self.fe is None:
            if self.__config.use_ela:
                ela = get_ela_feature(problem, self.population.group, self.population.cost)
                consumed_fes = ela[1]
            ...
            feature = self.fe(self.population.group, self.population.cost)[0]
            feature = feature.numpy().mean(0)
        # =======================================================================
    ...
    def update(self, action, problem):
        ...
        if self.done:
            if len(self.cost) >= self.n_logpoint + 1:

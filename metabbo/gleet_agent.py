...
import torch
from metabbo.networks import MultiHeadEncoder, MLP, EmbeddingNet, MultiHeadCompat
...
class GLEET_Agent(Basic_Agent):
    def __init__(self, config, use_feature_extractor = False):
        ...
        if use_feature_extractor:
            self.__config.node_dim = config.hidden_dim
        else:
            if self.__config.use_ela:
                self.__config.node_dim = 32
            else:
    ...
    def train_episode(self, env):
        ...
        while not done:
            ...
            for _k in range(K_epochs):
                ...
                # if self.__learn_steps >= self.__config.max_learning_step:

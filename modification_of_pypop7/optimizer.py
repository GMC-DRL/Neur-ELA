...
class Optimizer(object):
    ...
    def _evaluate_fitness(self, x, args=None):
        ...
        if args is None:
            ...
            y = self.fitness_function(x, *args)
        self.time_function_evaluations += time.time() - self.start_function_evaluations
        self.n_function_evaluations += ps
        # find the min y, compare it with the best_so_far
        min_pos = np.argmin(y)
        min_val = np.min(y)
        if min_val < self.best_so_far_y:
            self.best_so_far_x, self.best_so_far_y = np.copy(x[min_pos]), min_val
        return y

...
class Actor(nn.Module):
    ...
    def forward(self, obs, test=False):
        feature = obs['feature'][None, :]
        if not isinstance(feature, torch.Tensor):
            feature = torch.tensor(feature, dtype=torch.float).to(self.device)
        moves = []
        obs_move = obs['move']
        for i in range(len(self.embedders)):
            moves.append(self.embedders[i](torch.tensor([obs_move[i]], dtype=torch.float).to(self.device)))
...
class RL_DAS_FE_Agent(Basic_Agent):
    def __init__(self, config, fe=None):
        ...
        self.optimizer = torch.optim.Adam(
            [{'params': self.fe.parameters(), 'lr': 1e-3}] +
    ...
    def learn(self, memory, k_epoch, env):
        ...
        for k in range(k_epoch):
            if k == 0:
                ...
                for tt in range(length):
                    ...
                    old_states[tt]['feature'] = self.fe(x, y)[0].mean(0)
                    # !
            ...
            x = old_states[tt]['x']
            y = old_states[tt]['y']
            old_states[-1]['feature'] = self.fe(x, y)[0].mean(0)
            # !
    ...
    def train_episode(self, env):
        ...
        obs = env.reset()
        obs['feature'] = self.fe(obs['x'], obs['y'])[0].mean(0)
        ...
        self.learn(memory, k_epoch, env)

        return self.global_ls >= self.max_learning_step, {'normalizer': env.optimizer.cost_scale_factor,
                                                        'gbest': env.optimizer.population.gbest,
                                                        'return': R,
                                                        'learn_steps': self.global_ls}
    ...
    def rollout_episode(self, env):
        ...
        state = env.reset()
        state['feature'] = self.fe(state['x'], state['y'])[0].mean(0)
        done = False
        R = 0
        while not done:
